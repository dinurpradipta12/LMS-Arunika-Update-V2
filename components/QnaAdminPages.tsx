...
type QnaQuestionSortOrder = 'newest' | 'oldest';
...
const sortQuestionsForDisplay = (questions: QnaQuestion[], sortOrder: QnaQuestionSortOrder = 'newest') => (
  [...questions].sort((a, b) => {
    if (a.isPinned !== b.isPinned) return a.isPinned ? -1 : 1;
    const createdAtDifference = new Date(a.createdAt).getTime() - new Date(b.createdAt).getTime();
    return sortOrder === 'oldest' ? createdAtDifference : -createdAtDifference;
  })
);
...
const QnaModerationPanel: React.FC<{
  ...
  sortOrder: QnaQuestionSortOrder;
  onSortOrderChange: (sortOrder: QnaQuestionSortOrder) => void;
...
}> = ({ questions, isLoading, filter, onFilterChange, sortOrder, onSortOrderChange, answerDrafts, onAnswerChange, onUpdate, onDelete, onCreateCard }) => {
  const filteredQuestions = useMemo(() => sortQuestionsForDisplay(filter === 'all' ? questions : questions.filter(question => question.status === filter), sortOrder), [filter, questions, sortOrder]);
...
export const QnaAdminDetailPage: React.FC<{ client: any }> = ({ client }) => {
  ...
  const [questionSortOrder, setQuestionSortOrder] = useState<QnaQuestionSortOrder>('newest');
  const [isLoading, setIsLoading] = useState(true);
  ...
  return <div className="mx-auto max-w-7xl space-y-6 p-4 md:p-8" style={formThemeStyle(session.theme)}>
    ...
    {activeTab === 'moderation' && <QnaModerationPanel questions={questions} isLoading={isLoadingQuestions} filter={questionFilter} onFilterChange={setQuestionFilter} sortOrder={questionSortOrder} onSortOrderChange={setQuestionSortOrder} answerDrafts={answerDrafts} onAnswerChange={(questionId, value) => setAnswerDrafts(current => ({ ...current, [questionId]: value }))} onUpdate={(question, patch) => { void updateQuestion(question, patch); }} onDelete={question => setDeleteTarget({ kind: 'question', id: question.id, label: question.body })} onCreateCard={setSelectedQuestionForCard} />}
