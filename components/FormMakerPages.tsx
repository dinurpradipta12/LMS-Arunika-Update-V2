...
export const PublicFormView: React.FC<{ client: any; slugOverride?: string }> = ({ client, slugOverride }) => {
          ...
          <Card className="space-y-5 py-10 text-center">
            ...
            {mode === 'payment' && <div className="mx-auto grid w-full max-w-xl items-start gap-4 rounded-xl bg-[var(--surface-soft)] p-4 text-left sm:p-5 lg:grid-cols-[minmax(0,1fr)_220px]">
              <div className="space-y-4">
                ...
                {whatsappLink ? <a className="inline-flex w-full items-center justify-center gap-2 rounded-xl bg-[#128c7e] px-4 py-3 text-sm font-semibold text-white hover:brightness-95" href={whatsappLink} target="_blank" rel="noreferrer"><Check size={16} /> Saya sudah melakukan pembayaran</a> : <div className="space-y-2"><button type="button" disabled className="inline-flex w-full cursor-not-allowed items-center justify-center gap-2 rounded-xl bg-[#128c7e]/50 px-4 py-3 text-sm font-semibold text-white"><Check size={16} /> Saya sudah melakukan pembayaran</button><p className="text-xs leading-relaxed text-[var(--muted)]">Tombol akan aktif setelah nomor WhatsApp konfirmasi diatur oleh penyelenggara.</p></div>}
              </div>
