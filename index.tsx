  ...
  const fetchLatest = useCallback(async () => {
    ...
    try {
      ...
      const full: Course = {
        ...
        coverPosition: typeof c.cover_position === 'string' ? c.cover_position : '50% 50%',
        coverZoom: Math.max(100, Math.min(200, Number(c.cover_zoom) || 100)),
        coverTextMode: c.cover_text_mode === 'light' ? 'light' : 'dark',
  ...
  const fetchAllData = useCallback(async () => {
    ...
    try {
      ...
      setCourses((coursesResult.data || []).map((item: any) => ({
        ...
        coverPosition: typeof item.cover_position === 'string' ? item.cover_position : '50% 50%',
        coverZoom: Math.max(100, Math.min(200, Number(item.cover_zoom) || 100)),
        coverTextMode: item.cover_text_mode === 'light' ? 'light' : 'dark',
  ...
  const handleUpdateCourse = async (updatedCourse: Course, updatedMentor?: Mentor) => {
    ...
    try {
      const courseData = {
        ...
        cover_position: updatedCourse.coverPosition || '50% 50%',
        cover_zoom: Math.max(100, Math.min(200, Number(updatedCourse.coverZoom) || 100)),
        cover_text_mode: updatedCourse.coverTextMode === 'light' ? 'light' : 'dark',
